...
function getCart() { return JSON.parse(localStorage.getItem("tutty_pijamas_cart")) || []; }
function saveCart(cart) { localStorage.setItem("tutty_pijamas_cart", JSON.stringify(cart)); updateCartCount(); }
...
function updateCartCount() {
    ...
    if (cartCountElement) {
        ...
        if (totalItems > 0) { cartCountElement.textContent = totalItems; cartCountElement.style.display = "block"; } 
...
function renderFeaturedProducts() {
    ...
    if (featuredGrid) { const featured = products.slice(0, 4); featuredGrid.innerHTML = featured.map(renderProductCard).join(''); }
}
function renderProductGrid(productsToRender) {
    const productGrid = document.getElementById("product-grid");
    if (productGrid) {
        if (productsToRender.length === 0) { productGrid.innerHTML = "<p>Nenhum produto encontrado com os filtros selecionados.</p>"; return; }
...
function renderCartPage() {
    ...
    if (cart.length === 0) { cartContainer.innerHTML = "<div style='text-align:center; width:100%;'><h1 class='section-title'>Seu carrinho está vazio.</h1><a href='produtos.html' class='btn'>Ver produtos</a></div>"; return; }
    let subtotal = 0;
    cartItemsContainer.innerHTML = cart.map(item => {
        const itemTotal = item.price * item.quantity; subtotal += itemTotal;
        return `
            <tr>
                <td><div style="display: flex; align-items: center;"><img src="${item.images[0]}" alt="${item.name}"><span>${item.name}</span></div></td>
    ...
    cartSummaryContainer.innerHTML = `<h3>Resumo do Pedido</h3><p><span>Subtotal</span> <strong>R$ ${subtotal.toFixed(2).replace('.', ',')}</strong></p><p><span>Frete</span> <strong>R$ ${frete.toFixed(2).replace('.', ',')}</strong></p><hr><p><span>Total</span> <strong>R$ ${total.toFixed(2).replace('.', ',')}</strong></p><a href="checkout.html" class="btn">Finalizar Compra</a>`;
}
function updateQuantity(productId, quantity) {
    let cart = getCart(); const item = cart.find(p => p.id === productId);
    if (item) { item.quantity = parseInt(quantity, 10); if (item.quantity <= 0) { cart = cart.filter(p => p.id !== productId); } saveCart(cart); renderCartPage(); }
}
function removeFromCart(productId) {
    let cart = getCart(); cart = cart.filter(item => item.id !== productId); saveCart(cart); renderCartPage();
    }
