...
const uiService = {
    ...
    renderCartPage: function() {
        ...
        if (cart.length === 0) {
            ...
            if(summaryContainer) summaryContainer.innerHTML = ''; // Limpa o resumo se o carrinho estiver vazio
            return;
        }
                        ...
                        <td data-label="Produto">
                            <div class="cart-product-info">
                                ...
                                <div>
                                    <span>${item.name}</span>
                                    <br>
                                    <small>${item.selectedSize ? 'Tam: ' + item.selectedSize : ''}</small>
                                </div>
                            </div>
                        </td>
                        <td data-label="Preço">R$ ${item.price.toFixed(2).replace('.', ',')}</td>
                        <td data-label="Quantidade"><input class="cart-quantity-input" type="number" value="${item.quantity}" min="1" data-product-id="${item.cartItemId}"></td>
                        <td data-label="Total">R$ ${itemTotal.toFixed(2).replace('.', ',')}</td>
                        <td data-label="Remover"><button class="remove-from-cart-btn" data-product-id="${item.cartItemId}">Remover</button></td>
                    </tr>`;
        }).join('');
    ...
    showNotification: function(message, type = 'success') {
        ...
        if (!notification && (document.body.id === 'login-page' || document.body.id === 'checkout-page')) {
            console.warn("Elemento de notificação não encontrado na página de login/checkout.");
            alert(message); // Fallback se o elemento de notificação não existir
            return;
        }
        if (notification) {
    ...
    const App = {
            ...
            document.body.addEventListener('click', event => {
                ...
                if (document.body.id === 'product-detail-page') {
                    ...
                    if (event.target.matches('#add-to-cart-detail')) {
                        ...
                        if (!selectedSizeEl) {
                            uiService.showNotification('Por favor, selecione um tamanho.', 'error');
                            return;
                        }
        ...
        handlePageSpecifics() {
            ...
            switch (pageId) {
                ...
                case 'checkout-page': this.initCheckoutPage(); break;
            }
        },
        ...
        renderCheckoutSummary() {
            ...
            if (cart.length === 0) {
                itemsContainer.innerHTML = '<p>Seu carrinho de compras está vazio.</p>';
                 // Poderia desabilitar o botão de finalizar compra aqui também
        ...
        initCheckoutPage() {
            ...
            this.renderCheckoutSummary(); // Renderiza o resumo do pedido com itens do carrinho
            ...
            applyMask(inputs.cpf, cpfMask); applyMask(inputs.celular, celularMask); applyMask(inputs.cep, cepMask);
            applyMask(inputs.cardExpiry, cardExpiryMask); applyMask(inputs.cardNumber, cardNumberMask); applyMask(inputs.cardCvv, cvvMask);
            
            const showError = (inputElement, message) => {
                const formGroup = inputElement.closest('.form-group'); if (!formGroup) return;
                const errorElement = formGroup.querySelector('.error-message');
                if (errorElement) errorElement.textContent = message; inputElement.classList.add('input-error');
            };
            const clearError = (inputElement) => {
                const formGroup = inputElement.closest('.form-group'); if (!formGroup) return;
                const errorElement = formGroup.querySelector('.error-message');
                if (errorElement) errorElement.textContent = ''; inputElement.classList.remove('input-error');
            };
            ...
            const validators = {
                ...
                cardExpiry: value => {
                    ...
                    const [month, year] = value.split('/'); const currentYear = new Date().getFullYear() % 100;
                    const currentMonth = new Date().getMonth() + 1;
                    if (parseInt(year) < currentYear || (parseInt(year) === currentYear && parseInt(month) < currentMonth)) return "Cartão expirado.";
                    if (parseInt(month) < 1 || parseInt(month) > 12) return "Mês inválido."; return null;
                },
            ...
            const validateField = (inputElement) => {
                ...
                if (!inputElement.hasAttribute('required') && !inputElement.value.trim()) { clearError(inputElement); return true; }
                if (!inputElement.value.trim() && inputElement.hasAttribute('required')){ showError(inputElement, "Campo obrigatório."); return false; }
                const validator = validators[inputElement.id];
                if (validator) { const error = validator(inputElement.value); if (error) { showError(inputElement, error); return false; } }
            ...
            const validateAllFields = () => {
                ...
                if (inputs.endereco && !inputs.endereco.disabled) { fieldsToValidate.push('endereco', 'numero', 'bairro', 'cidade', 'estado'); }
                fieldsToValidate.forEach(id => { if (inputs[id] && !validateField(inputs[id])) { allValid = false; if (!firstInvalidField) firstInvalidField = inputs[id]; } });
            ...
            Object.values(inputs).forEach(input => { if(input) input.addEventListener('input', () => { validateField(input); validateAllFields(); }); });
            paymentMethodRadios.forEach(radio => {
                radio.addEventListener('change', () => {
                    Object.values(paymentDetailsDivs).forEach(div => { if(div) div.classList.remove('active'); });
                    const activeDiv = paymentDetailsDivs[radio.value]; if (activeDiv) activeDiv.classList.add('active');
                    Object.keys(inputs).forEach(id => { if (inputs[id] && inputs[id].closest('.payment-details') && !inputs[id].closest('.payment-details.active')) { clearError(inputs[id]); } });
                    validateAllFields(); 
                });
            });
            if(buscarCepBtn) buscarCepBtn.addEventListener('click', () => {
                ...
                if (inputs.cep && /^\d{5}-\d{3}$/.test(inputs.cep.value)) {
                    ...
                    ['endereco', 'numero', 'complemento', 'bairro', 'cidade', 'estado'].forEach(id => { if(inputs[id]) inputs[id].disabled = false; });
                    if(inputs.endereco) inputs.endereco.value = "Rua dos Pijamas Felizes"; if(inputs.bairro) inputs.bairro.value = "Centro Sonolento";
                    if(inputs.cidade) inputs.cidade.value = "Cidade dos Sonhos"; if(inputs.estado) inputs.estado.value = "SC"; if(inputs.numero) inputs.numero.value = "123";
                     ['endereco', 'bairro', 'cidade', 'estado', 'numero'].forEach(id => { if(inputs[id]) validateField(inputs[id]); });
                    validateAllFields();
                } else if (inputs.cep) { showError(inputs.cep, "CEP inválido."); }
            });
            const applyCouponBtn = document.getElementById('apply-coupon-btn');
            if(applyCouponBtn) applyCouponBtn.addEventListener('click', () => {
                const couponCodeInput = document.getElementById('coupon-code');
                if (couponCodeInput && couponCodeInput.value.trim()) uiService.showNotification(`Cupom "${couponCodeInput.value}" aplicado! (Simulação)`);
                else uiService.showNotification('Por favor, insira um código de cupom.', 'error');
            });
            if(inputs.cardNumber && cardBrandIcon) inputs.cardNumber.addEventListener('input', () => {
                const num = inputs.cardNumber.value.replace(/\s/g, ''); let iconClass = '';
                if (/^4/.test(num)) iconClass = 'fab fa-cc-visa'; else if (/^5[1-5]/.test(num)) iconClass = 'fab fa-cc-mastercard';
                else if (/^3[47]/.test(num)) iconClass = 'fab fa-cc-amex'; else if (/^(5067|4576|5090|6362|6363|4389|5041|4514|6277)/.test(num)) iconClass = 'fas fa-credit-card';
                cardBrandIcon.className = `card-brand-icon ${iconClass}`;
            });
            if(copyPixBtn) copyPixBtn.addEventListener('click', () => {
                const pixCodeInput = document.getElementById('pix-code');
                if (pixCodeInput) {
                    pixCodeInput.select(); pixCodeInput.setSelectionRange(0, 99999); 
                    try { document.execCommand('copy'); copyPixBtn.innerHTML = '<i class="fas fa-check"></i> Copiado!'; setTimeout(() => { copyPixBtn.innerHTML = '<i class="fas fa-copy"></i> Copiar'; }, 2000); }
                    catch (err) { uiService.showNotification('Erro ao copiar o código Pix.', 'error'); }
                }
            });
            document.querySelectorAll('.wallet-btn').forEach(btn => { btn.addEventListener('click', (e) => { uiService.showNotification(`Iniciando pagamento com ${e.target.textContent.trim()}... (Simulação)`); }); });
            form.addEventListener('submit', (e) => {
                e.preventDefault(); const validationResult = validateAllFields(); 
                if (validationResult.allValid) {
                    const selectedPaymentMethodRadio = form.querySelector('input[name="payment-method"]:checked'); let paymentMethodText = "Não selecionado";
                    if (selectedPaymentMethodRadio) {
                        switch(selectedPaymentMethodRadio.value) {
                            case 'card': paymentMethodText = 'Cartão de Crédito'; break; case 'pix': paymentMethodText = 'Pix'; break;
                            case 'boleto': paymentMethodText = 'Boleto Bancário'; break; case 'wallet': paymentMethodText = 'Carteira Digital'; break;
                        }
                    }
                    uiService.showNotification(`Pedido realizado com sucesso via ${paymentMethodText}! Obrigado, ${inputs.nome.value}!`);
                    cartService.clearCart(); setTimeout(() => window.location.href = 'index.html', 3000); 
                } else { uiService.showNotification('Por favor, corrija os erros no formulário.', 'error'); if (validationResult.firstInvalidField) validationResult.firstInvalidField.focus(); }
            });
            validateAllFields();
        }
    };
